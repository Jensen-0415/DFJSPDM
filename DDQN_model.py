...
class DoubleDQN(nn.Module):
    def __init__(self, inDim, outDim, BATCH_SIZE, LR , EPSILON, GAMMA, MEMORY_CAPACITY,TARGET_REPLACE_ITER):
        ...
        self.eval_net = Net(inDim, outDim)
        self.target_net = Net(inDim, outDim)
        self.optimizer = torch.optim.Adam(self.eval_net.parameters(), lr=LR)
        self.memory = np.zeros((MEMORY_CAPACITY, self.N_STATES * 2 + 2))
        self.loss_func = nn.MSELoss()


        # Move networks and loss function to GPU if available
    ...
    def choose_action(self, x):
        ...
        if np.random.uniform() < self.EPSILON:   # greedy
            ...
            index=0;max_v=actions_value_[0]
            for i in range(1,self.N_ACTIONS):
                if max_v<actions_value_[i]:
                    index=i;max_v=actions_value_[i]
    ...
    def learn(self):
        ...
        b_s = b_s.cuda() #current state
        b_a = b_a.cuda() #current action
        b_r = b_r.cuda() #current reward
        b_s_ = b_s_.cuda() #next state
        #double DQN
        # q_eval w.r.t the action in experience
        q_eval = self.eval_net(b_s).gather(1, b_a)  # shape (batch, 1)
        q_next1 = self.eval_net(b_s_).detach() #use current Q net to esmate next action's Q value
        _,act_pre=q_next1.max(1)# find the action with max q value
        act_pre=act_pre.view(self.BATCH_SIZE, 1)
        q_next2 = self.target_net(b_s_).detach() #use target Q net to esmate next action's Q value
        q_next2=q_next2.gather(1, act_pre) #use qvalue with the action selected by current Q
        q_target = b_r + self.GAMMA * q_next2.view(self.BATCH_SIZE, 1)   # shape (batch, 1)
